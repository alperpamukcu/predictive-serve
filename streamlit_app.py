...
def _render_about_dialog() -> None:
    ...
    st.markdown(
        ...
        **Predictive Serve** is a transparent, end-to-end tennis match
        ...
        - All artifacts (predictions CSV, model PKL, player metadata,
          photos) are regenerated by a **GitHub Actions cron that runs
          twice a day — 04:00 and 14:00 UTC** — and committed straight
          to the repo, so the deployed app is always fresh without
          manual steps. The afternoon slot exists so the Upcoming +
          Live tabs don't drift through European prime time.
    ...
    # --- Data Coverage ---
    # Honest breakdown of what we actually have vs what's missing.
    # Helps a recruiter / interviewer instantly understand the data's
    # completeness without diving into CSVs.
    try:
        ...
        if not pred.empty:
            total = len(pred)
            with_market = int(pred["pA_market"].notna().sum()) if "pA_market" in pred.columns else 0
            yr = pd.to_datetime(pred["date"], errors="coerce").dt.year
            cov_rows.append(f"- **Match coverage**: {total:,} matches scored, span {int(yr.min())}–{int(yr.max())}")
            cov_rows.append(f"- **Bookmaker odds coverage**: {with_market:,} / {total:,} matches ({with_market/total*100:.1f}%) — older seasons + smaller events go without")
        if not history.empty:
            with_rank = int(pd.to_numeric(history.get("rankA"), errors="coerce").notna().sum()) if "rankA" in history.columns else 0
            unique_players = pd.concat([history.get("playerA", pd.Series(dtype=str)), history.get("playerB", pd.Series(dtype=str))]).dropna().nunique()
            unique_tours = history["tournament"].dropna().nunique() if "tournament" in history.columns else 0
            cov_rows.append(f"- **Roster size**: {unique_players:,} unique players across {unique_tours:,} tournaments")
            cov_rows.append(f"- **ATP rank coverage**: {with_rank:,} matches carry the WTA/ATP rank at match time")
        # Player metadata cache size
        cache_size = 0
        try:
            cache = _load_player_cache()
            cache_size = sum(1 for m in cache.values() if m.full_name)
        except Exception:
            pass
        if cache_size:
            cov_rows.append(f"- **Player metadata cache**: {cache_size:,} players with full name + flag + photo cached from API-Tennis")
        # Photo coverage
        try:
            photo_dir = ASSETS_DIR / "players"
            n_photos = len(list(photo_dir.glob("*.jpg"))) if photo_dir.exists() else 0
            if n_photos:
                cov_rows.append(f"- **Player photos**: {n_photos:,} downloaded and tracked in the repo")
        except Exception:
            pass
        if cov_rows:

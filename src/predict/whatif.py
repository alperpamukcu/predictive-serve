...
def get_player_snapshot(
    ...
    # Form columns have 'A_5' or 'B_5' pattern (e.g. form_winrateA_5)
    # The simple safe() appends prefix at end, which fails for these.
    f5_key = f"form_winrate{prefix}_5"
    f10_key = f"form_winrate{prefix}_10"
    
    form5 = float(last[f5_key]) if f5_key in last and pd.notna(last[f5_key]) else np.nan
    form10 = float(last[f10_key]) if f10_key in last and pd.notna(last[f10_key]) else np.nan
...
def build_feature_row(
    ...
    # Use explicit snapshot dates if provided, otherwise fallback to match date
    d_A = snapshot_dateA if snapshot_dateA is not None else date
    d_B = snapshot_dateB if snapshot_dateB is not None else date
    
    snapA = get_player_snapshot(history, playerA, d_A, surface)
    snapB = get_player_snapshot(history, playerB, d_B, surface)
    ...
    # This prevents "Nadal vs Unknown" becoming "Average vs Average" (50%) due to Imputer.
    
    val_eloA = row["eloA"] if pd.notna(row.get("eloA")) else 1500.0
    val_eloB = row["eloB"] if pd.notna(row.get("eloB")) else 1500.0
    row["elo_diff"] = val_eloA - val_eloB
    
    # Also fill back the NaNs in the row so the imputer doesn't overwrite them with Mean later?
    # Actually, let's just ensure the DIFF is correct, because the Model relies mostly on Diff.
    # But if we leave row["eloA"] as NaN, imputer will fill it with 1900 (mean). 
    # That's fine, as long as Diff is +600.
    
    # Update Surface Elo Diffs too
